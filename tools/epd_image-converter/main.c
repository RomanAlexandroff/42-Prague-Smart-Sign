...
const char *szOptions[] = {"BW", "BWR", "BWY", "BWYR", "4GRAY", "16GRAY", NULL};
...
//
void WriteBMP(uint8_t *pBitmap, int cx, int cy, int bpp, FILE *oHandle)
{
int i, bsize, lsize;
uint32_t *l;
uint8_t ucTemp[1024];
...
int iHeaderSize;
   ...
   *l = (uint32_t)i; /* Store the file size */
    l = (uint32_t *)&pHdr[10]; // offset to the pixels
    i -= (cy * lsize); // header + palette size
    *l = (uint32_t)i;
   ...
   *(l+1) = (uint32_t)cy;  /* height */
   fwrite(pHdr, 1, iHeaderSize, oHandle);
    if (bpp <= 8) { // create a grayscale palette
        ...
        fwrite(ucTemp, 1, (4<<bpp), oHandle);
    } 
   /* Write the image data */
   for (i=cy-1; i>=0; i--)
    {
        s = &pBitmap[i*bsize];
        if (0) {  //bpp >= 24) { // swap R/B for Windows BMP byte order
            uint8_t ucTemp[2048];
            int j, iBpp = bpp/8;
            uint8_t *d = ucTemp;
            for (j=0; j<cx; j++) {
...
//
// Return the given pixel as a 4-bit grayscale value
//
unsigned char GetGrayPixel4(int x, int y, uint8_t *pData, int iPitch, int iBpp)
{
    uint8_t uc=0, *s;
    int r=0, g=0, b=0;
    ...
    switch (iBpp) {
        ...
        case 4:
            ...
            g = ucGreen[uc];
            r = ucRed[uc];
            uc = (uint8_t)((b + g + r*2) >> 6); // simple grayscale
            break;
        case 8:
            s = &pData[(y * iPitch) + x];
            uc = s[0];
...
//
// Convert 4-bit grayscale (16GRAY) into hex 1-plane output
//
void MakeC_16GRAY(uint8_t *pSrc, int iOffBits, int iWidth, int iHeight, int iBpp, int iSize, FILE *ohandle, char *szLeaf)
{
    int iSrcPitch, iPitch;
    int i, x, y, iIn, iTotal, iLine;
    uint8_t uc, *s = &pSrc[iOffBits];

    iSrcPitch = ((iBpp * iWidth) + 7)/8;
    iSrcPitch = (iSrcPitch + 3) & 0xfffc; // Windows BMP lines are dword aligned
    iPitch = (iWidth + 1)/2; // bytes per line
    iTotal = iPitch * iHeight; // how many bytes we're creating
    // show pitch
    fprintf(ohandle, "// Image size: width %d, height %d\n", iWidth, iHeight);
    fprintf(ohandle, "// %d bytes per line\n", iPitch);
    ...
    for (y=0; y<iHeight; y++) {
        for (x=0; x<iWidth; x+=2) {
            uc = GetGrayPixel4(x, y, s, iSrcPitch, iBpp) << 4; // slower, but easier on the eyes
            uc |= GetGrayPixel4(x+1, y, s, iSrcPitch, iBpp);
            i++;
            iLine++;
            fprintf(ohandle, "0x%02x", uc);
            if (i != iTotal) {
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3 || argc > 6)
    {
        ...
        printf("PREVIEW = create output as a BMP file to see how it will look\n");
        ...
        printf("16GRAY = create output for 4-bit grayscale displays\n");
    ...
    while (argv[iNameParam][0] == '-') { // check options
        if (strcmp(argv[iNameParam], "ROTATE") == 0) {
            ...
            bMSBFirst = 0;
        } else if (strcmp(argv[iNameParam], "--PREVIEW") == 0) {
            bPreview = 1;
    ...
    if (bPreview) {
       WriteBMP(&p[iOffBits], iWidth, iHeight, iBpp, ihandle);
       free(p);
       return 0;
    }
    ...
    switch (iOption) {
	    ...
	    break;
            case OPTION_16GRAY:
               MakeC_16GRAY(p, iOffBits, iWidth, iHeight, iBpp, iSize, ihandle, szLeaf);
            break;
    } // switch
    fflush(ihandle);
    fclose(ihandle);
